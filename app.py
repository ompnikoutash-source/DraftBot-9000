...
def mc_on_clock_key(
    ...
    return f"{current_pick}|{my_owner}|{','.join(draft_order)}|" + ",".join(ids)
...
def simulate_finish(
    ...
    by_id = pool.set_index("player_id")[["player_id", "player", "pos", "team", "proj_points", "vorp"]].to_dict(orient="index")
    ...
    def available_df():
        ids = [pid for pid in by_id.keys() if pid not in taken]
        if not ids:
            return pd.DataFrame(columns=pool.columns)
        return pool[pool["player_id"].isin(ids)].copy()
