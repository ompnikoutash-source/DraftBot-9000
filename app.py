...
def parse_previous_draft_results(xlsx_path: str) -> Dict[int, pd.DataFrame]:
    """
    Reads Previous Draft Results blocks.
    In your workbook, each year starts on a row where column B has the year (2016, 2017, 2018, 2025).
    That row also contains the 12 owners in columns C to N.
    Rounds are in column B below that, and positions in columns C to N.
    """
    df = pd.read_excel(xlsx_path, sheet_name="Previous Draft Results", header=None)

    # Find year rows by scanning column 1 (B in Excel, 0-based index 1)
    year_rows = []
    for idx, val in df[1].items():
        if pd.notna(val) and isinstance(val, (int, float)) and float(val).is_integer():
            iv = int(val)
            if 1900 <= iv <= 2100:
                year_rows.append((iv, idx))

    blocks: Dict[int, pd.DataFrame] = {}
    for i, (yr, start_idx) in enumerate(year_rows):
        end_idx = year_rows[i + 1][1] - 1 if i + 1 < len(year_rows) else len(df) - 1
        block = df.iloc[start_idx : end_idx + 1].copy()

        owners = block.iloc[0, 2:14].tolist()
        rounds = block.iloc[1:, 1]
        mat = block.iloc[1:, 2:14]
