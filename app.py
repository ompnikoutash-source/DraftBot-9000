...
# Default owner order for the 2025 block in your file
DEFAULT_DRAFT_ORDER_2025 = [
...
class LeagueSettings:
    ...
    def __post_init__(self):
        if self.starters is None:
            # Your lineup:
            # QB, WR, WR, RB, RB, W/R, TE, K, DEF, Bench x5
            self.starters = {
                "QB": 1,
                "RB": 2,
                "WR": 2,
...
def parse_player_projections_from_workbook(xlsx_path: str) -> pd.DataFrame:
    """
    Parses your Player Projections sheet into a uniform player pool:
    player, pos, team, proj_points, player_id
    Works even when name+pos+team are concatenated in the first column.
    """
    df = pd.read_excel(xlsx_path, sheet_name="Player Projections")
    player_col = df.columns[0]

    # Find section header rows where first column equals "Player"
    header_idx = df.index[df[player_col].astype(str).str.strip().eq("Player")].tolist()

    # Find defense header row where first column equals "Team" (and it contains Sack/Int, etc)
    def_header_idx = df.index[df[player_col].astype(str).str.strip().eq("Team")].tolist()
    def_header_idx = def_header_idx[0] if def_header_idx else None

    if len(header_idx) < 2:
        raise ValueError("Could not find Player section headers in Player Projections tab.")

    # Expected structure in your sheet:
    # QB section: header_idx[0]
    # RB section: header_idx[1]
    # WR section: header_idx[2]
    # TE section: header_idx[3]
    # K section: header_idx[4]
    # DEF section: starts after def_header_idx
    ranges: List[Tuple[str, int, int]] = []
    if len(header_idx) >= 5:
        ranges.append(("QB", header_idx[0] + 1, header_idx[1] - 1))
        ranges.append(("RB", header_idx[1] + 1, header_idx[2] - 1))
        ranges.append(("WR", header_idx[2] + 1, header_idx[3] - 1))
        # TE ends a couple rows before K header lines appear
        ranges.append(("TE", header_idx[3] + 1, header_idx[4] - 2))
        k_start = header_idx[4] + 1
        k_end = (def_header_idx - 1) if def_header_idx is not None else (len(df) - 1)
        ranges.append(("K", k_start, k_end))
    else:
        # Fallback: treat everything as one pool with Fantasy column
        ranges.append(("ALL", header_idx[0] + 1, len(df) - 1))
    ...
    for pos, start, end in ranges:
        ...
        sub = sub[sub[player_col].notna()]
        if sub.empty:
            continue

        # Choose points column by section
        # QB/RB/WR/TE: Fantasy column contains projected points
        # K: column labeled "Points" sits in "Unnamed: 7"
        # DEF: "Points" sits in column "Ret"
        if pos in ["QB", "RB", "WR", "TE", "ALL"]:
            pts_col = "Fantasy"
        elif pos == "K":
            pts_col = "Unnamed: 7"
        else:  # DEF
        ...
        for _, r in sub.iterrows():
            raw = _clean_raw_name(r[player_col])
            # Split team from " - "
            team = None
            if " - " in raw:
            ...
            # Extract position suffix at end of left string, like "Lamar JacksonQB"
            m = re.search(r"(QB|RB|WR|TE|K|DEF)$", left)
            if m:
            ...
            # Use section position unless ALL fallback
            final_pos = detected_pos if pos == "ALL" else pos
    ...
    pool["team"] = pool["team"].astype(str).str.replace(r"\s+$", "", regex=True)
    pool["player_id"] = (
        pool["player"].str.lower().str.replace(r"[^a-z0-9]+", "_", regex=True).str.strip("_")
        + "_"
        + pool["pos"]
    )
    # Keep only positions we care about
    pool = pool[pool["pos"].isin(POSITIONS)].copy()
...
def compute_vorp_with_flex(pool: pd.DataFrame, settings: LeagueSettings) -> Tuple[pd.DataFrame, Dict[str, float]]:
    """
    Computes replacement points and VORP, handling RB/WR FLEX correctly.
    Uses projected season points in proj_points.
    """
    df = pool.copy()
...
def init_state():
    ...
    if "draft_order" not in st.session_state:
        st.session_state.draft_order = DEFAULT_DRAFT_ORDER_2025.copy()
...
def draft_player(row: pd.Series, owner: str, vorp_row: pd.Series):
                ...
                [
                    {
                        ...
                        "pos": row["pos"],
                        "team": row["team"],
                        "proj_points": float(vorp_row["proj_points"]),
                        "vorp": float(vorp_row["vorp"]),
                        "ts": time.time(),
                    }
                ]
            ),
...
def run_risk(settings: LeagueSettings, history: pd.DataFrame, draft_log: pd.DataFrame, available_df: pd.DataFrame) -> Dict[str, float]:
    """
    ...
    recent_score = {p: recent_counts.get(p, 0) / max(1, min(window, len(recent))) for p in POSITIONS}

    # Historical round distribution
    ...
    # Scarcity proxy: fewer strong remaining options means more danger
    scarcity = {}
    for p in POSITIONS:
...
def need_bonus(my_owner: str, settings: LeagueSettings) -> Dict[str, float]:
    ...
    for p in POSITIONS:
        ...
        bonus[p] = 1.0 * deficit
    # Small bias toward RB/WR depth because of FLEX and bench utility
    bonus["RB"] += 0.25
    bonus["WR"] += 0.20
    return bonus
...
xlsx_path = st.sidebar.text_input("Workbook path", st.session_state.xlsx_path)
...
settings.history_year = st.sidebar.selectbox("History year for run tendencies", year_options, index=year_options.index(settings.history_year) if settings.history_year in year_options else 0)

st.session_state.my_owner = st.sidebar.selectbox("My owner", DEFAULT_DRAFT_ORDER_2025, index=DEFAULT_DRAFT_ORDER_2025.index(st.session_state.my_owner) if st.session_state.my_owner in DEFAULT_DRAFT_ORDER_2025 else 0)

st.session_state.auto_advance = st.sidebar.checkbox("Auto advance owner (snake)", value=st.session_state.auto_advance)

use_history_order = st.sidebar.checkbox("Use draft order from selected history year", value=True)
if use_history_order:
    hist = history_blocks[settings.history_year]
    hist_order = hist.columns.tolist()[1:]
    # enforce string + strip
    hist_order = [str(x).strip() for x in hist_order]
    st.session_state.draft_order = hist_order
else:
    st.sidebar.caption("Custom order (comma-separated):")
    custom = st.sidebar.text_area("Draft order", value=",".join(st.session_state.draft_order))
    st.session_state.draft_order = [x.strip() for x in custom.split(",") if x.strip()]
...
with top_right:
    ...
    st.metric("Next pick", pick)
    st.metric("Round", rnd)
    st.session_state.current_owner = st.selectbox("Currently picking owner", st.session_state.draft_order, index=st.session_state.draft_order.index(st.session_state.current_owner))
    c1, c2 = st.columns(2)
    with c1:
        st.button("Undo last pick", on_click=undo_last_pick)
    with c2:
        if st.button("Advance to next owner"):
            next_pick = next_pick_number(st.session_state.draft_log)
            idx = snake_team_index(next_pick, settings.num_teams)
            st.session_state.current_owner = st.session_state.draft_order[idx]
...
with left:
    ...
    show_n = st.slider("Show top N per position", 10, 75, 35)

    # Simple pill-like buttons
    st.markdown(
        """
        <style>
        div.stButton > button {
    ...
    for i, pos in enumerate(POSITIONS):
        with tabs[i]:
            sub = pool_vorp[(pool_vorp["pos"] == pos)].copy()
            if search.strip():
            ...
            # 3 columns of bubbles
            cols = st.columns(3)
            for j, (_, rrow) in enumerate(sub.iterrows()):
                disabled = not bool(rrow["available"])
                label = f"{rrow['player']} ({rrow['team']})  |  {pos}  |  VORP {rrow['vorp']:.1f}"
                with cols[j % 3]:
                    if st.button(label, key=f"{pos}_{rrow['player_id']}", disabled=disabled):
                        draft_player(rrow, st.session_state.current_owner, rrow)
                        st.rerun()
...
with right:
    ...
    if not av.empty:
        ...
        st.success(f"{best['player']} ({best['pos']}) | Adj {best['adjusted']:.1f} | VORP {best['vorp']:.1f} | Proj {best['proj_points']:.1f}")
    else:
...
c1, c2 = st.columns(2)
with c1:
    if st.button("Export Draft Log to CSV"):
        out = st.session_state.draft_log.sort_values("pick")
        out.to_csv("draft_log_export.csv", index=False)
        st.success("Saved draft_log_export.csv in the app folder.")
with c2:
    st.caption(f"Replacement points: QB {repl['QB']:.1f} | RB {repl['RB']:.1f} | WR {repl['WR']:.1f} | TE {repl['TE']:.1f} | FLEX {repl['FLEX_RBWR']:.1f} | K {repl['K']:.1f} | DEF {repl['DEF']:.1f}")
